...
- `home.html` (`home.ts`) — the account hub: profile (rename, sign out), games in progress with a your-move badge, incoming/outgoing challenges (accept / decline / cancel), friends list + requests. Live-refreshes via `subscribeSocial`
- `game.html` (`game.ts`) — the per-variant lobby: start an open game, challenge a friend (picker over accepted friends), join a listed open game. Also the `?join=<id>` handoff target: sign in, claim the seat, land on the live board
