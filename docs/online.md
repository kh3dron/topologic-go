...
- `games.ts` — Edge Function invokers (`create-game` with optional opponent, `join-game`, `cancel-game`, `submit-move` with `expected_ply`, `gameAction` for resign/draw offers), `fetchGame`, `listOpenGames` (excludes challenges), `listMyGames`, `listActiveGames` (spectator browse), `listGamesForStats` / `listMyGamesForStats` (narrow-column rows for derived stats + achievements), `subscribeGame` (Realtime)
...
- `online.ts` — `enterOnlineGame(id)`: loads authoritative state into the game wrapper via the view's `loadState`, gates input to the seated color (`setOnline`), submits moves optimistically, reconciles on Realtime updates, renders the `#online-banner`. While active, seated players get Offer draw / Accept / Decline / Retract buttons driven by `games.draw_offer`, plus a two-click Resign (first click arms, disarms after 4s); a rejected action re-syncs before surfacing the message. The done banner reads `board_state.gameOver` to say "by resignation" / "by agreement" when the end was off-board, and carries a Replay button (any viewer, including signed-out spectators) that scrubs the finished game with first/prev/slider/next/last controls; while scrubbing, server updates keep their bookkeeping but leave board + banner to the scrubber until Exit
- `replay.ts` — `makeReplay(variant, topology, finalBoardState, moves)`: recomputes any position by replaying the world-readable moves log through the shared engine from the canonical initial state (Go's board size recovered from the final snapshot). Positions are served through a forward cursor — stepping ahead is incremental, backward jumps replay from the start — so no per-ply snapshots accumulate (a Go snapshot embeds the whole superko `seen` list; a materialized timeline would be quadratic) The banner also carries the share/copy-link button (creator), a join button (signed-in visitor), or a sign-in handoff link (signed-out visitor). Two ambient cues per server update: the tab title gains a `● Your move —` prefix while it's the seated player's turn (restored on destroy), and an opponent Go stone landing plays the stone sound (new ply that leaves the turn with us + non-null `lastMove`, so passes and our own move's Realtime echo stay silent)
...
- `migrations/` — schema, applied in filename order: game-agnostic tables + RLS + signup trigger + realtime (`init`), atomic move RPC (`apply_move`), friendships + `games.invited_player` + username rules (`social`), `game_types` registration of later games (`hyperchess`), snake best-score table (`snake_scores`), resign/draw support: `games.draw_offer` + `apply_move` clearing it on every applied move (`game_actions`), Elo rating trigger (`ratings`: `profiles.rating`, K=32, draws 0.5, fires on the games row's active→done transition so mate, resign and agreed draws all rate; historical done games are not retro-rated). Registering a game in code without the matching `game_types` INSERT fails at create-game with a `games_variant_fkey` violation
- `functions/` — the only privileged game writers:
  ...
  - `game-action(game_id, action)` — off-board actions on an active game by a seated player: `resign` (works on or off turn; opponent wins), `offer-draw` (sets `games.draw_offer` to the offerer), `accept-draw` (ends the game, `winner` null), `decline-draw` (clears the offer; the offerer calling it is a retract). Never touches `board_state`, so a game ended here keeps `board_state.gameOver` null — that is how clients tell resignation/agreement apart from mate or a scored board. All writes are conditional updates (status still active, offer still present/absent), so races with moves or the other player's action lose cleanly with a 409
  - `submit-move(game_id, expected_ply, move)` — validates with the shared engine, applies atomically via the `apply_move` RPC. After the commit it fires per-move web push, best-effort: notifies whoever must act next (the non-mover at game end) via `_shared/webpush.ts` — RFC 8291 aes128gcm + RFC 8292 VAPID implemented directly on WebCrypto, no npm dependency. Sends race a 3s timeout, 404/410 subscriptions are deleted, and the whole thing no-ops when `VAPID_*` env is absent. Subscriptions live in `push_subscriptions` (RLS: players manage only their own rows; client flow in `src/net/push.ts` + `public/sw.js`, toggle on the account hub). VAPID keys: private in `selfhost/.env`, public in the frontend build as `VITE_VAPID_PUBLIC_KEY` (repo variable) — the two must match
