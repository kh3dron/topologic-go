...
- Playground (`/play.html?g=<game>&t=<topo>`):
  ...
  - Keyboard: chess and Go are fully playable with arrows + Enter/Space (Escape hides the cursor); the cursor's cell carries `.kb-cursor` on every displayed copy, so drivers can `page.keyboard.press(...)` instead of computing cell indices
