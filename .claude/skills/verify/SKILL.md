...
- Overlay: `#topology-overlay`, `.topo-tile` (classes `reflected`, `seam-*`, `wall-*`), `.topo-label` text is ORIGINAL / REFLECTED / ROTATED 90|180|270 (glyph/label only on tessellated topologies; classic gets just the wall border). Legend rows: `#seam-legend .legend-row`. OFF by default — check the sidebar `#show-boundaries` checkbox (`#boundary-control`, visible on all topology games, offline and online) before asserting overlay content.
